...
def _find_word_bbox(words, search_text):
    ...
    # currency-stripped match (p.sh. "$10,000.00" vs "10000.0")
    for w in words:
        if w["text"].strip() == search_str:
            return _word_to_bbox(w)

    # try matching without currency symbols
    ...
    # numeric comparison (p.sh. 12000.0 vs "$12,000.00")
    try:
...
def extract_bounding_boxes_from_pdf(pdf_path, extracted_data):
    ...
    for field in BBOX_FIELDS:
        ...
        # dates get reformatted during extraction
...
def write_line_items_csv(extracted_data, csv_path):
    ...
    line_confidence = round(
        sum(scores.get(k, 0) for k in [
            "line_item_description", "line_item_quantity", "line_item_unit_price"]) / 3, 2
    )
    po_ref = extracted_data.get("po_reference")

    fieldnames = ["line_no", "description", "quantity",
                  "unit_price", "total", "po_line_ref", "confidence"]
...
def run_extraction(bundle_path, run_dir=None):
    ...
    # extract duplicate invoice for Agent G history
...
if __name__ == "__main__":
    ...
    parser.add_argument("--bundle-dir", required=True,
                        help="Path to the input bundle directory")
